...
### OA 下載實際命中率遠低於預測（2026-08-02 實跑發現）

**現象**：`--download-oa` 首跑結果 7/19 = **2/26**、8/2 = **1/29**，遠低於當初實測的 **45.8%**。

**根因 = 驗證方法的盲點**（重要教訓）：
當初那個 45.8% 是 **dry-run 量「有沒有解析出一個 PDF URL」**，**從沒驗「那個 URL 真的下載得到 PDF」**。
實跑才發現 URL 解析得出來、但**下載那步大量失敗**：

| 失敗型態 | 來源 | 佔比（8/2 觀察） |
|---|---|---|
| `europepmc.org/api/getPdf` 回 500/404 | 我的 PMC 路由（`?pdf=render` 會 302 轉到這個會壞的 endpoint，實測確認） | 多 |
| `pmc.ncbi.nlm.nih.gov/.../pdf` 403 | S2 給的 PMC 直連（NCBI 擋程式爬 PDF） | 多 |
| 出版商頁 403 / 回 HTML | OUP(AJRCCM)、LWW(CCM)、Elsevier(Lancet) 的落地頁 | 多（`%PDF` 驗證正確擋下，非抓到假檔） |

**能救的部分（＝真正的修法，若要做）**：
- **PMC 路由改走 NCBI OA Web Service**（`https://www.ncbi.nlm.nih.gov/pmc/utils/oa/oa.fcgi?id=PMCxxx`）：它**只回真正在 OA subset、可下載**的檔的實際位置（PDF 或 .tar.gz），一舉解決「europepmc render 壞」＋「pmc.ncbi 直連 403」兩種——因為它給的是合法開放的下載點，不是擋爬蟲的網頁。這是最高 CP 值的一刀。
- 出版商落地頁（OUP/LWW/Elsevier）403/HTML 那批**救不了**，除非走 TDM key 或機構代理（＝我們決定不碰的層）。所以修好 PMC 後，天花板大概是 20–30%，**不會回到 46%**——46% 本來就是假數字。

**評估要點**：
- 值不值得做，看修好 PMC 後的真實增益（可先寫唯讀探測：把 8/2 那 29 篇的 PMCID 丟 oa.fcgi 看幾篇真的給得出下載點）。**這次記得量端到端、別再量 URL 解析。**
- 現況**非緊急**：抓到的都是真 PDF、下游 organize/KB 正常；只是自動化涵蓋率比想像低，多數全文仍要手動。

> ⚠️ 教訓（寫死在這免得再犯）：**量「解析出 URL」≠ 量「下載到 PDF」。** 驗證自動化功能一定要量**端到端的最終產物**，不是中間步驟的成功率。這跟 7/15 journal「測試要用真實資料、量真實那件事」是同一個坑，我在同一條 flow 上踩了第二次。
...
**BMJ 已處理（同日）**：補漏 W28–W36 各週過濾後 3/2/3/3/3/1/5/2/1 篇，PubMed 收錄延遲實測 0–1 天
（不是索引沒補齊，是 BMJ 過濾後就這麼少）。連續出版期刊改取「最新已結束、≥1 篇的週」，
不套 ≥3 門檻、不退路。副作用：BMJ 週摘要有時只有 1–2 篇，屬實情。
