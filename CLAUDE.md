...
| 用途 | 路徑 |
|--|--|
| 工具本體 | `~/lybrish_claude/sandbox/pdf-download/` |
...
├── naming.py           # PDF 改名規則（沿用 med-literature-organizer skill）
├── journals/
...
- **命名規則**：以 `med-literature-organizer` skill 為準（`naming.py` 沿用）
...
`scripts/` 下有：
- `run_fetch.sh` — fetch wrapper：venv + fetch + log + macOS 通知（給 launchd 排程觸發）
- `com.lybrish.pdf-download.plist` — launchd 設定（每週日 16:00）
- `install_schedule.sh` — 含 inbox 寫入預檢的安裝器
- `uninstall_schedule.sh` — 卸載
- `PDF-Organize.command` — 雙擊執行 organize 的 .command 檔（透過 Terminal）

### 為什麼用 .command 不是 .app

原本想做 .app 雙擊（無 Terminal 視窗）但 macOS 對 unsigned `.app` 的
TCC 處理在 14+ 變得很挑剔，FDA 開關開了還是常常被擋。
`.command` 透過 Terminal（已經有 FDA）執行，繞開這個問題。
代價：Terminal 視窗會閃一下，但功能完整。

PDF-Organize.command 安裝在 `~/Applications/PDF-Organize.command`，
可拖到 Dock 右側（檔案區）做一鍵啟動。

排程觸發後會：
1. cd 進專案目錄、啟用 venv
2. 跑 `pdf-download fetch`（state file 會自動跳過已抓的期）
